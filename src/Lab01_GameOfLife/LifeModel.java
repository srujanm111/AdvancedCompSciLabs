...
public class LifeModel implements ActionListener {
     ...
     */
    private void oneGeneration() throws IndexOutOfBoundsException {
        for (int r = 0; r < SIZE; r++) {
            for (int c = 0; c < SIZE; c++) {
                int neighbors = 0;
                if (checkNeighbor(r, c, 0, -1)) neighbors++;
                if (checkNeighbor(r, c, 0, 1)) neighbors++;
                if (checkNeighbor(r, c, 1, -1)) neighbors++;
                if (checkNeighbor(r, c, 1, 0)) neighbors++;
                if (checkNeighbor(r, c, 1, 1)) neighbors++;
                if (checkNeighbor(r, c, -1, -1)) neighbors++;
                if (checkNeighbor(r, c, -1, 0)) neighbors++;
                if (checkNeighbor(r, c, -1, 1)) neighbors++;
    ...
    private boolean checkNeighbor(int r, int c, int rOffset, int cOffset) {
        return grid[r + rOffset][c + cOffset].isAliveNow();
    }
}
